...
def handler_verify_password(plain: str, salt_hex: str, hash_hex: str):
    return auth.verify_password(plain, salt_hex, hash_hex)

@app.get("/")
def handler_login_backoff_seconds(username: str):
    return auth._login_backoff_seconds(username)

@app.get("/")
def handler_record_login_failure(username: str):
    return auth._record_login_failure(username)

@app.put("/")
def handler_reset_login_failures(username: str):
    return auth._reset_login_failures(username)

# Cookie Funcions
@app.post("/")
def handler_set_csrf_cookie(resp: Response):
    return auth.set_csrf_cookie(resp)

@app.post("/")
def handler_csrf_protect(request: Request, csrf_cookie: Annotated[Optional[str], Cookie(alias=CSRF_COOKIE)] = None):
    return auth.csrf_protect(request, csrf_cookie, Cookie) # something is wrong with this
...
def handler_create_new_user(username: str, password: str): #type-casting it to User breaks the function
    return auth.create_new_user(username, password)

@app.get("/")
def handler_validate_credentials(username: str, password: str):
    return auth.validate_credentials(username, password)

@app.get("/")
def handler_db_get_session_user(sessionid: str):
    return auth.db_get_session_user(sessionid)

@app.get("/")
def handler_check_authorization(userid: int, resource: str):
    return auth.check_authorization(userid, resource)

@app.post("/")
def handler_seed_if_empty():
    return auth._seed_if_empty()
...
def handler_convert_to_csstring(lst: Iterable[str] | None):
    return part.convert_to_csstring(lst)

@app.put("/")
def handler_cstring_to_list(cstring: str | None):
    return part.cstring_to_list(cstring)

@app.post("/")
def handler_exec(sql: str, params: Tuple | dict = ()):
    return part._exec(sql, params)

@app.get("/")
def handler_row_to_particle(row):
    return part._row_to_particle(row)  

@app.get("/")
def handler_init_particles_fts():
    return part._init_particles_fts()

# _init_particles_fts()

@app.get("/")
def handler_countParticles(query: str = ""):
    return part.countParticles(query)
...
def handler_update_particle(
    ...
    return part.update_particle(particle_id, title, body, tags, particle_references
) 
    
@app.put("/")
def delete_particle(particle_id: int):
    return part.delete_particle(particle_id)

@app.post("/")
def handler_create_Particle(title: str, body: str):
    return part.create_Particle(title, body)

@app.get("/")
def handler_listParticles(query: str = "", page: int = 1, pageSize: int = 10):
    return part.listParticles(query, page, pageSize)

@app.get("/")
def handler_getParticle(id: int):
    return part.getParticle(id)

@app.get("/")
def handler_view_particles(query: str = "", page: int = 1, pageSize: int = 10):
    return part.view_particles(query, page, pageSize) 

@app.get("/")
def handler_extract_tags_and_particle_refs(particle: Particle):
    return part.extract_tags_and_particle_refs(particle)

# Storage Module
@app.post("/")
def handler_store_user(user: User):
    return stor.store_user(user)

@app.post("/")
def handler_store_particle(particle: Particle):
    return stor.store_particle(particle)
