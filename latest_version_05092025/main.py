...
# User Module
@app.get("/")
async def handler_hex(b: bytes):
    return pim._hex(b)

@app.get("/")
async def handler_unhex(s: str):
    return pim._unhex(s)

@app.get("/")
async def handler_hash_password(plain: str):
    return pim.hash_password(plain)

@app.get("/")
async def handler_verify_password(plain: str, salt_hex: str, hash_hex: str):
    return pim.verify_password(plain, salt_hex, hash_hex)

@app.get("/")
async def handler_login_backoff_seconds(username: str):
    return pim._login_backoff_seconds(username)

@app.get("/")
async def handler_record_login_failure(username: str):
    return pim._record_login_failure(username)

@app.put("/")
async def handler_reset_login_failures(username: str):
    return pim._reset_login_failures(username)

# Cookie Funcions
@app.post("/")
async def handler_set_csrf_cookie(resp: Response):
    return pim.set_csrf_cookie(resp)

@app.post("/")
async def handler_csrf_protect(request: Request, csrf_cookie: Annotated[Optional[str], Cookie(alias=CSRF_COOKIE)] = None):
...
@app.get("/")
async def sign(text: str):
    return pim.sign(text)

@app.get("/") 
async def handler_login(username: str, password: str,  *, user_agent: Optional[str] = None, ip: Optional[str] = None):
    return pim.login(username, password, user_agent, ip)

@app.put("/")
async def handler_logout(session_token: str):
    return pim.logout(session_token)

@app.post("/")
async def handler_create_new_user(username: str, password: str): #type-casting it to User breaks the function
    return pim.create_new_user(username, password)

@app.get("/")
async def handler_validate_credentials(username: str, password: str):
    return pim.validate_credentials(username, password)

@app.get("/")
async def handler_db_get_session_user(sessionid: str):
    return pim.db_get_session_user(sessionid)

@app.get("/")
async def handler_check_authorization(userid: int, resource: str):
    return pim.check_authorization(userid, resource)

@app.post("/")
async def handler_seed_if_empty():
...
# Particle Module
@app.put("/")
async def handler_convert_to_csstring(lst: Iterable[str] | None):
    return pim.convert_to_csstring(lst)

@app.put("/")
async def handler_cstring_to_list(cstring: str | None):
    return pim.cstring_to_list(cstring)

@app.post("/")
async def handler_exec(sql: str, params: Tuple | dict = ()):
    return pim._exec(sql, params)

@app.get("/")
async def handler_row_to_particle(row):
    return pim._row_to_particle(row)  

@app.get("/")
async def handler_init_particles_fts():
    return pim._init_particles_fts()

# _init_particles_fts()

@app.get("/")
async def handler_countParticles(query: str = ""):
...
@app.put("/")
async def delete_particle(particle_id: int):
    return pim.delete_particle(particle_id)

@app.post("/")
async def handler_create_Particle(title: str, body: str):
    return pim.create_Particle(title, body)

@app.get("/")
async def handler_listParticles(query: str = "", page: int = 1, pageSize: int = 10):
    return pim.listParticles(query, page, pageSize)

@app.get("/")
async def handler_getParticle(id: int):
    return pim.getParticle(id)

@app.get("/")
async def handler_view_particles(query: str = "", page: int = 1, pageSize: int = 10):
    return pim.view_particles(query, page, pageSize) 

@app.get("/")
async def handler_extract_tags_and_particle_refs(particle: Particle):
    return pim.extract_tags_and_particle_refs(particle)

# Storage Module
@app.post("/")
async def handler_store_user(user: User):
    return pim.store_user(user)

@app.post("/")
async def handler_store_particle(particle: Particle):
